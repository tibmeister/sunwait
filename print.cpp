...
double diurnalArcWithOffset1(const double pDiurnalArc, const double pOffset)
{
  ...
  if (arcWithOffset >= 24.0)
    return 24.0;
  if (arcWithOffset <= 0.0)
    return 0.0;
...
double getOffsetRiseHourUTC1(const double pSouthHourUTC,
                             const double pDiurnalArc, const double pOffsetHour)
{
...
{
  return getOffsetRiseHourUTC1(pTarget->southHourUTC, pTarget->diurnalArc,
                               pRun->offsetHour);
}

// What time, in hours UTC, is the offset sunset?
double getOffsetSetHourUTC1(const double pSouthHourUTC,
                            const double pDiurnalArc, const double pOffsetHour)
{
  return pSouthHourUTC + diurnalArcWithOffset1(pDiurnalArc, pOffsetHour) / 2.0;
}
...
{
  return getOffsetSetHourUTC1(pTarget->southHourUTC, pTarget->diurnalArc,
                              pRun->offsetHour);
}

void print_a_time(const OnOff pGmt_OnOff, const time_t *pMidnightTimet,
...
void print_a_sun_time(const OnOff pGmt_OnOff, const time_t *pMidnightTimet,
                      const double pEventHour, const double pOffsetDiurnalArc)
...
void print_times(const OnOff pGmt, const OnOff pSunrise, const OnOff pSunset,
                 const OnOff pSolarnoon, const time_t pMidnightTimet,
                 const double pSouthHour, const double pDiurnalArc,
                 const double pOffset, const char *pSeparator)
{
...
inline void print_times(const runStruct *pRun, const targetStruct *pTarget,
                        const double pOffsetHour, const char *pSeparator)
{
  print_times(pRun->utc, pRun->reportSunrise, pRun->reportSunset,
              pRun->reportSolarnoon, pRun->targetTimet, pTarget->southHourUTC,
              pTarget->diurnalArc, pOffsetHour, pSeparator);
}

inline void print_twilight(const double pDayLength,
                           const double pTwilightLength)
{
  printf("%2.2d:%2.2d hours (twilight: %2.2d:%2.2d hours)\n", hours(pDayLength),
         minutes(pDayLength), hours(pTwilightLength), minutes(pTwilightLength));
}
...
void generate_report(const runStruct *pRun)
{
  ...
  */
  ...
  printf("Sun direct N/S (solar noon): ");
  print_a_time(pRun->utc, &pRun->targetTimet, tmpTarget.southHourUTC);
  printf("\n");

  if (pRun->offsetHour != NO_OFFSET) {
    printf("                     Offset: %2.2d:%2.2d hours\n",
           hours(pRun->offsetHour), minutes(pRun->offsetHour));
  }

  if (tmpTarget.twilightAngle == TWILIGHT_ANGLE_DAYLIGHT)
    printf("             Twilight angle: %5.2f degrees (daylight)\n",
           twilightAngleTarget);
  else if (tmpTarget.twilightAngle == TWILIGHT_ANGLE_CIVIL)
    printf("             Twilight angle: %5.2f degrees (civil)\n",
           twilightAngleTarget);
  else if (tmpTarget.twilightAngle == TWILIGHT_ANGLE_NAUTICAL)
    printf("             Twilight angle: %5.2f degrees (nautical)\n",
           twilightAngleTarget);
  else if (tmpTarget.twilightAngle == TWILIGHT_ANGLE_ASTRONOMICAL)
    printf("             Twilight angle: %5.2f degrees (astronomical)\n",
           twilightAngleTarget);
  else
  ...
  print_times(pRun, &tmpTarget, NO_OFFSET, cTo);

  if (pRun->offsetHour != NO_OFFSET) {
    printf(" Day with twilight & offset: ");
    print_times(pRun, &tmpTarget, pRun->offsetHour, cTo);
  }
  ...
  */
  ...
  print_times(pRun, &civilTarget, NO_OFFSET, cTo);
  printf("     with Nautical twilight: ");
  print_times(pRun, &nauticalTarget, NO_OFFSET, cTo);
  printf(" with Astronomical twilight: ");
  print_times(pRun, &astronomicalTarget, NO_OFFSET, cTo);
  printf("\n");
...
void print_list(const runStruct *pRun)
{
  ...
  for (unsigned int day = 0; day < pRun->listDays; day++) {
    sunriset(pRun, &tmpTarget);
    print_times(pRun, &tmpTarget, pRun->offsetHour, cComma);
