...
// IFC  2014-12-01  0.5  The linux port was not working - the windows sleep is
// for seconds, the linux is miliseconds.
//                       I'm porting my home power control to Linux, so I've
//                       reworked the whole program to suit.
...
/*
** Define global:
** 'runStruct' structure allows pretty much everything to be carted simply
*around functions.
** 'targetStruct' structure is the interface to the sunrise calcs. Compact, and
*allows multi-results to be used.
...
void print_version()
{
  ...
  printf(
      "Code Contributors: P.Schlyter, D.Risacher, D.MacMillan and I.Craig.\n");
  printf("\n");
  printf(" Sunset is the instant at which the upper edge of the Sun disappears "
         "below the horizon.\n");
  printf(" Civil twilight is the period from sunset until the geometric centre "
         "of the sun is 6� below the horizon.\n");
  printf(" Nautical twilight is the period when the geometric centre of the "
         "sun is between 6� and 12� below the horizon.\n");
  printf(" Astronomical twilight is the period when the geometric centre of "
         "the sun is between 12� and 18� below the horizon.\n");
  printf(" Night is period when the geometric center of the sun falls 18� "
         "below the horizon.\n");
}
...
void print_usage()
{
  ...
  printf("\n");
  printf("The program can either: wait for sunrise or sunset   (function: "
         "wait),\n");
  printf("  or return the time (GMT or local) the event occurs (function: "
         "list),\n");
  printf("  or report the day length and twilight timings      (function: "
         "report),\n");
  printf("  or simply report if it is DAY or NIGHT             (function: "
         "poll).\n");
  printf("\n");
  printf("You should specify the latitude and longitude of your target "
         "location.\n");
  printf("\n");
  printf("\n");
  printf("Usage: sunwait [major options] [minor options] [twilight type] "
         "[rise|set] [offset] [latitude] [longitude]\n");
  printf("\n");
  printf("Major options, either:\n");
  printf("    poll          Returns immediately indicating DAY or NIGHT. See "
         "'program exit codes'. Default.\n");
  printf("    wait          Sleep until specified event occurs. Else exit "
         "immediate.\n");
  printf("    list [X]      Report twilight times for next 'X' days "
         "(inclusive). Default: %u.\n",
         DEFAULT_LIST);
  printf("    report        Generate a report about the days sunrise and "
         "sunset timings.\n");
  printf("\n");
  printf("Minor options, any of:\n");
  printf("    [no]debug     Print extra info and returns in one minute. "
         "Default: nodebug.\n");
  ...
  printf("    angle [X.XX]  User-specified twilight-angle (degrees). Default: "
         "0.\n");
  printf("    solarnoon     Sun directly overhead.\n");
  printf("\n");
  printf("Sunrise/sunset. Only useful with major-options: 'wait' and 'list'. "
         "Any of: (default: both)\n");
  printf("    rise          Wait for the sun to rise past specified twilight & "
         "offset.\n");
  printf("    set           Wait for the sun to  set past specified twilight & "
         "offset.\n");
  printf("\n");
  printf("Offset:\n");
  printf("    offset [MM|HH:MM] Time interval (+ve towards noon) to adjust "
         "twilight calculation.\n");
  printf("\n");
  printf("Target date. Only useful with major-options: 'report' or 'list'. "
         "Default: today\n");
  printf("    d [DD]        Set the target Day-of-Month to calculate for. 1 to "
         "31.\n");
  printf("    m [MM]        Set the target Month to calculate for. 1 to 12.\n");
  printf("    y [YYYY]      Set the target Year to calculate for. 2000 to "
         "2099.\n");
  printf("\n");
  printf("latitude/longitude coordinates: floating-point degrees, with [NESW] "
         "appended. Default: Bingham, England.\n");
  printf("\n");
  ...
  printf("Wait until 1 hour 15 minutes 10 secs before the sun rises in "
         "Greenwich, London.\n");
  printf("\n");
  printf("Example 2: sunwait list 7 civil 55.752163N 37.617524E\n");
  printf("List civil sunrise and sunset times for today and next 6 days. "
         "Moscow.\n");
  printf("\n");
  printf("Example 3: sunwait poll exit angle 10 54.897786N -1.517536E\n");
  printf("Indicate by program exit-code if is Day or Night using a custom "
         "twilight angle of 10 degrees above horizon. Washington, UK.\n");
  printf("\n");
  printf("Example 4: sunwait list 7 gmt sunrise angle 3\n");
  printf("List next 7 days sunrise times, custom +3 degree twilight angle, "
         "default location.\n");
  printf("Uses GMT; as any change in daylight saving over the specified period "
         "is not considered.\n");
  printf("\n");
  printf("Note that program uses C library functions to determine time and "
         "localtime.\n");
...
boolean myIsSignedFloat(const char *arg)
{
  ...
  for (int i = 0;; i++) {
    ...
    case '9':
      ...
      break;
    case '.':
      break; /* Can be anywhere (but in front of sign), or not there */
    case '+':
    case '-':
      if (i > 0)
        return false;
...
boolean myIsSignedFloat(const char *pArg, double *pDouble)
{
  ...
  for (int i = 0;; i++) {
    ...
    case '\0': /* Exit */
      ...
      if (exponentSet && exponent > 0)
        number = number / pow(10, (double)exponent);
      if (negative)
        number = -number;
      *pDouble = number;
...
boolean isBearing(runStruct *pRun, const char *pArg)
{
  ...
  for (int i = 0;; i++) {
    ...
    case '\0': /* Exit */
      ...
      if (exponentSet && exponent > 0)
        bearing = bearing / pow(10, (double)exponent);
      ...
      if (compass == 'N')
        pRun->latitude = fixLatitude(bearing);
      else if (compass == 'E')
        pRun->longitude = fixLongitude(bearing);
      else
        return false;
      return true; /* All done */
      break;
...
boolean isOffset(runStruct *pRun, const char *pArg)
{
  ...
  for (int i = 0;; i++) {
    switch (pArg[i]) {
    case '0':
      number0 = (number0 * 10) + 0;
      break;
    case '1':
      number0 = (number0 * 10) + 1;
      break;
    case '2':
      number0 = (number0 * 10) + 2;
      break;
    case '3':
      number0 = (number0 * 10) + 3;
      break;
    case '4':
      number0 = (number0 * 10) + 4;
      break;
    case '5':
      number0 = (number0 * 10) + 5;
      break;
    case '6':
      number0 = (number0 * 10) + 6;
      break;
    case '7':
      number0 = (number0 * 10) + 7;
      break;
    case '8':
      number0 = (number0 * 10) + 8;
      break;
    case '9':
      number0 = (number0 * 10) + 9;
      break;
    case ':':
      number2 = number1;
      number1 = number0;
    ...
    case '\0': /* Exit */
      if (colon == 0)
        returnOffset = number0 / 60.0;
      else if (colon == 1)
        returnOffset = number1 + number0 / 60.0;
      else if (colon == 2)
        returnOffset = number2 + number1 / 60.0 + number0 / 3600.0;
      else
        return false;
      if (negativeOffset) {
...
double getUtcBiasHours(const time_t *pTimet)
{
  ...
  // Keep to the same day given, but go for noon. Daylight savings changes
  // usually happen in the early hours. mktime() changes the values in "struct
  // tm", so I need to use a private one anyway.
  utcTm.tm_hour = 12;
  utcTm.tm_min  = 0;
  utcTm.tm_sec  = 0;

  // Now convert this time to time_t (which is always, by definition, UTC),
  // so I can run both of the two functions I can use that differentiate between
  // timezones, using the same UTC moment.
  time_t noonTimet = mktime(
      &utcTm); // Unfortunately this is noonTimet is local time. It's the best I
               // can do. If it was UTC, all locations on earth are within the
               // same day at noon. (Because UTC = GMT.  Noon GMT +/- 12hrs
               // nestles upto, but not across, the dateline) Local-time 'days'
               // (away from GMT) will probably cross the date line.
  ...
  // This is not nice, but Visual Studio does not support "strftime(%z)" (get
  // UTC bias) like linux does. I'll figure out the UTC bias by comparing
  // readings of local time to UTC for the same moment.

  // Although localTm and utcTm may be different, some secret magic ensures that
  // mktime() will bring them back to the same time_t value (as it should: they
  // identify the same moment, just in different timezones). I'll just have to
  // work out the utcBias using the differing 'struct tm' values. It isn't
  // pretty.

  utcBiasHours = (localNoonTm.tm_hour - utcNoonTm.tm_hour) +
                 (localNoonTm.tm_min - utcNoonTm.tm_min) / 60.0;

  // The day may be different between the two times, especially if the local
  // timezone is near the dateline. Rollover of tm_yday (from 365 to 0) is a
  // further problem, but no bias is ever more than 24 hours - that wouldn't
  // make sense.

  if (localNoonTm.tm_year > utcNoonTm.tm_year)
    utcBiasHours += 24.0; // Local time is in a new year, utc isn't:     so
                          // local time is a day ahead
  else if (localNoonTm.tm_year < utcNoonTm.tm_year)
    utcBiasHours -= 24.0; // Local time is in old year, utc is new year: so
                          // local time is a day behind
  else
...
void myDebugTime(const char *pTitleChar, const time_t *pTimet)
{
  if (pRun->debug == ONOFF_ON) {
    ...
    printf("Debug: %s UTC bias (add to %s to get %s) hours: %f\n", pTitleChar,
           utcBuffer, localBuffer, getUtcBiasHours(pTimet));
  }
}
...
time_t getMidnightUTC(const time_t *pTimet, const runStruct *pRun)
{
  ...
  // Convert target "struct tm" to time_t.  It'll be set to midnight local time,
  // on the target day.
  ...
  tmpTm.tm_wday = 0;   // mktime will ignore this when calculating time_t as it
                       // contradicts days and months
  tmpTm.tm_yday = 0;   // mktime will ignore this when calculating time_t as it
                       // contradicts days and months
  tmpTm.tm_isdst = -1; // -1 means: mktime() must work it out. 0=DST not in
                       // effect. 1=DST in effect. (Daylight Savings)

  // Shave off (add) UTC offset, so that time_t is converted from midnight
  // local-time to midnight UTC on the target day
...
**
** THE TARGET DATE MUST BE THE CURRENT DATE (on 1st call, as this function
*iterates)
*/
...
{ // If the time is before sunrise or after sunset, I need to know that
  ...
  // Figure out if we're between sunrise and sunset (with offset) of any of the
  // three days
  return ((nowHourUTC >= riseHourUTCYesterday &&
           nowHourUTC <= setHourUTCYesterday) ||
          (nowHourUTC >= riseHourUTCToday && nowHourUTC <= setHourUTCToday) ||
          (nowHourUTC >= riseHourUTCTomorrow &&
           nowHourUTC <= setHourUTCTomorrow))
             ? ONOFF_ON
             : ONOFF_OFF;
}
...
int main(int argc, char *argv[])
{
  ...
  //
  // DO NOT PUT CODE BEFORE "debug" ARGUMENT IS KNOWN (UNTIL AFTER COMMAND LINE
  // PARSED)
  //
  ...
  */
  ...
  for (int i = 1; i < argc; i++) {
    ...
    if (!strcmp(arg, "v") || !strcmp(arg, "version"))
      pRun->functionVersion = ONOFF_ON;
    else if (!strcmp(arg, "nv") || !strcmp(arg, "noversion"))
      pRun->functionVersion = ONOFF_OFF;

    else if (!strcmp(arg, "?") || !strcmp(arg, "usage") || !strcmp(arg, "h") ||
             !strcmp(arg, "help"))
      pRun->functionUsage = ONOFF_ON;
    else if (!strcmp(arg, "nh") || !strcmp(arg, "nousage") ||
             !strcmp(arg, "nohelp"))
      pRun->functionUsage = ONOFF_OFF;

    // Major Options
    else if (!strcmp(arg, "wait"))
      pRun->functionWait = ONOFF_ON;
    else if (!strcmp(arg, "poll"))
      pRun->functionPoll = ONOFF_ON;
    else if (!strcmp(arg, "report"))
      pRun->functionReport = ONOFF_ON;
    ...
    else if (!strcmp(arg, "gmt") || !strcmp(arg, "utc"))
      pRun->utc = ONOFF_ON;
    else if (!strcmp(arg, "nogmt") || !strcmp(arg, "noutc"))
      pRun->utc = ONOFF_OFF;
    ...
    else if (!strcmp(arg, "e") || !strcmp(arg, "er") || !strcmp(arg, "exit") ||
             !strcmp(arg, "poll") || !strcmp(arg, "exitreport"))
      pRun->functionPoll = ONOFF_ON;
    else if (!strcmp(arg, "ne") || !strcmp(arg, "ner") ||
             !strcmp(arg, "noexit") || !strcmp(arg, "nopoll") ||
             !strcmp(arg, "noexitreport"))
      pRun->functionPoll = ONOFF_OFF;

    // Specify twilight angle
    else if (!strcmp(arg, "sun") || !strcmp(arg, "day") ||
             !strcmp(arg, "light") || !strcmp(arg, "normal") ||
             !strcmp(arg, "visible") || !strcmp(arg, "daylight"))
      pRun->twilightAngle = TWILIGHT_ANGLE_DAYLIGHT;
    else if (!strcmp(arg, "civil") || !strcmp(arg, "civ"))
      pRun->twilightAngle = TWILIGHT_ANGLE_CIVIL;
    else if (!strcmp(arg, "nautical") || !strcmp(arg, "nau") ||
             !strcmp(arg, "naut"))
      pRun->twilightAngle = TWILIGHT_ANGLE_NAUTICAL;
    else if (!strcmp(arg, "astronomical") || !strcmp(arg, "ast") ||
             !strcmp(arg, "astr") || !strcmp(arg, "astro"))
      pRun->twilightAngle = TWILIGHT_ANGLE_ASTRONOMICAL;
    else if (!strcmp(arg, "a") || !strcmp(arg, "angle") ||
             !strcmp(arg, "twilightangle") || !strcmp(arg, "twilight")) {
    ...
    else if (!strcmp(arg, "y") && i + 1 < argc && myIsNumber(argv[i + 1]))
      yearInt = atoi(argv[++i]); // Note: "++i"
    else if (!strcmp(arg, "m") && i + 1 < argc && myIsNumber(argv[i + 1]))
      monInt = atoi(argv[++i]); // Note: "++i"
    else if (!strcmp(arg, "d") && i + 1 < argc && myIsNumber(argv[i + 1]))
      mdayInt = atoi(argv[++i]); // Note: "++i"

    // Specify fixed duration offset
    else if (!strcmp(arg, "o") || !strcmp(arg, "off") ||
             !strcmp(arg, "offset")) {
      if (i + 1 < argc && isOffset(pRun, argv[i + 1])) {
        ++i;
      } /* Functionality in "isOffset()" */
    }

    // Specify if sunrise and/or sunset timings a required.
    else if (!strcmp(arg, "sunrise") || !strcmp(arg, "rise") ||
             !strcmp(arg, "dawn") || !strcmp(arg, "sunup") ||
             !strcmp(arg, "up"))
      pRun->reportSunrise = ONOFF_ON;
    else if (!strcmp(arg, "sunset") || !strcmp(arg, "set") ||
             !strcmp(arg, "dusk") || !strcmp(arg, "sundown") ||
             !strcmp(arg, "down"))
      pRun->reportSunset = ONOFF_ON;

    else if (!strcmp(arg, "noon") || !strcmp(arg, "solarnoon"))
      pRun->reportSolarnoon = ONOFF_ON;

    // Specify latitude and longitude - quite complex, so I've handled it
    // elsewhere
    else if (isBearing(pRun, arg)) {
    } /* Functionality in "isBearing()" */
  ...
  */
  ...
  time(&pRun->nowTimet);
  if (pRun->debug == ONOFF_ON)
    myDebugTime("Now", &pRun->nowTimet);

  pRun->utcBiasHours = getUtcBiasHours(&pRun->nowTimet);
  if (pRun->debug == ONOFF_ON)
    printf("Debug: UTC Bias (hours): %f\n", pRun->utcBiasHours);

  pRun->now2000 = daysSince2000(&pRun->nowTimet);
  if (pRun->debug == ONOFF_ON)
    printf("Debug: Now: Days since 2000: %lu\n", pRun->now2000);
  ...
  */

  {
    ...
    // I'll get the local-time day, as it'll make sense with the user, unless
    // UTC was asked for
    //
    ...
    if (pRun->debug == ONOFF_ON)
      printf("Debug: Target  year set to: %u\n", targetTm.tm_year);
    ...
    if (pRun->debug == ONOFF_ON)
      printf("Debug: Target   mon set to: %u\n", targetTm.tm_mon);
    ...
    if (pRun->debug == ONOFF_ON)
      printf("Debug: Target  mday set to: %u\n", targetTm.tm_mday);
    ...
    // Fiddle with other "struct tm" fields to get things right
    targetTm.tm_wday = 0; // mktime will ignore this when calculating time_t as
                          // it contradicts days and months
    targetTm.tm_yday = 0; // mktime will ignore this when calculating time_t as
                          // it contradicts days and months
    targetTm.tm_isdst = -1; // -1 means: mktime() must work it out. 0=DST not in
                            // effect. 1=DST in effect. (Daylight Savings)

    // Convert target "struct tm" to time_t.  It'll be set to midnight local
    ...
    // see
    // https://sourceforge.net/p/sunwait4windows/discussion/general/thread/98fc17dd5f/
    struct tm localTm;
    ...
    // All done
    pRun->targetTimet = mktime(&targetTm); // <<<<<< The important bit done <<<
                                           // targetTimet is set to midnight UTC

    if (pRun->debug == ONOFF_ON)
      myDebugTime("Target", &pRun->targetTimet);
  }

  pRun->target2000 = daysSince2000(&pRun->targetTimet);
  if (pRun->debug == ONOFF_ON)
    printf("Debug: Target: Days since 2000: %lu\n", pRun->target2000);
  ...
  */
  ...
  if (pRun->latitude == NOT_SET) {
    if (pRun->debug == ONOFF_ON)
      printf("Debug: latitude not set. Default applied.\n");
    pRun->latitude = DEFAULT_LATITUDE; /* The Buttercross, Bingham, England */
  }

  if (pRun->longitude == NOT_SET) {
    if (pRun->debug == ONOFF_ON)
      printf("Debug: longitude not set. Default applied.\n");
    pRun->longitude = DEFAULT_LONGITUDE; /* The Buttercross, Bingham, England */
  }

  // /* Co-ordinates must be in 0 to 360 range */           // IFC 2014-12-02:
  // Removed as done in isBearing() pRun->latitude  = revolution
  // (pRun->latitude);   // IFC 2014-12-02: Removed as done in isBearing()
  // pRun->longitude = revolution (pRun->longitude);  // IFC 2014-12-02: Removed
  // as done in isBearing()
  ...
  */
  ...
  if (pRun->twilightAngle == NOT_SET) {
    if (pRun->debug == ONOFF_ON)
      printf("Debug: twilight angle not set. Default: daylight.\n");
    pRun->twilightAngle = TWILIGHT_ANGLE_DAYLIGHT;
  }

  if (pRun->twilightAngle <= -90 || pRun->twilightAngle >= 90) {
    printf("Error: Twilight angle must be between -90 and +90 (-ve = below "
           "horizon), your setting: %f\n",
           pRun->twilightAngle);
    pRun->twilightAngle = TWILIGHT_ANGLE_DAYLIGHT;
  }

  if (pRun->debug == ONOFF_ON) {
    if (pRun->twilightAngle == TWILIGHT_ANGLE_DAYLIGHT)
      printf("Debug: Twilight - Daylight\n");
    else if (pRun->twilightAngle == TWILIGHT_ANGLE_CIVIL)
      printf("Debug: Twilight - Civil\n");
    else if (pRun->twilightAngle == TWILIGHT_ANGLE_NAUTICAL)
      printf("Debug: Twilight - Nautical\n");
    else if (pRun->twilightAngle == TWILIGHT_ANGLE_ASTRONOMICAL)
      printf("Debug: Twilight - Astronomical\n");
    else
      printf("Debug: Twilight - Custom angle (degrees): %f\n",
             pRun->twilightAngle);
  }
  ...
  */
  ...
  if (pRun->debug == ONOFF_ON)
    printf("Debug: User specified offset (hours): %f\n", pRun->offsetHour);
  ...
  */
  ...
  if (pRun->functionList == ONOFF_OFF && pRun->functionPoll == ONOFF_OFF &&
      pRun->functionUsage == ONOFF_OFF && pRun->functionVersion == ONOFF_OFF &&
      pRun->functionWait == ONOFF_OFF && pRun->functionReport == ONOFF_OFF)
    pRun->functionUsage = ONOFF_ON;
  ...
  */
  ...
  if (pRun->functionVersion == ONOFF_ON) {
    if (pRun->debug == ONOFF_ON)
      printf("Debug: Function selected: Version\n");
    print_version();
    exitCode = EXIT_OK;
  }
  ...
  if (pRun->functionUsage == ONOFF_ON) {
    if (pRun->debug == ONOFF_ON)
      printf("Debug: Function selected: Usage\n");
    print_usage();
    exitCode = EXIT_OK;
  }
  ...
  if (pRun->functionReport == ONOFF_ON) {
    if (pRun->debug == ONOFF_ON)
      printf("Debug: Function selected: Report\n");
  ...
  if (pRun->functionList == ONOFF_ON) {
    if (pRun->debug == ONOFF_ON)
      printf("Debug: Function selected: List\n");
    print_list(pRun);
    exitCode = EXIT_OK;
  }
  ...
  if (pRun->functionWait == ONOFF_ON) {
    if (pRun->debug == ONOFF_ON)
      printf("Debug: Function selected: Wait\n");
    exitCode = wait(pRun);
  }

  if (pRun->functionPoll == ONOFF_ON) {
    if (pRun->debug == ONOFF_ON)
      printf("Debug: Function selected: Poll\n");
    exitCode = poll(pRun);
    if (exitCode == EXIT_DAY)
      printf("DAY\n");
    else if (exitCode == EXIT_NIGHT)
      printf("NIGHT\n");
...
/*
** Simply check if we think now/current-time is night OR day (day includes
*twilight)
*/
...
** Midnight UTC can lands later and later within a local day for large
*longitudes.
** Latitudes heading towards the poles can either shrink the day length to
*nothing or the whole day, depending on the season.
...
int wait(const runStruct *pRun)
{
  ...
  ** For latitudes near poles, the sun might not pass through specified twilight
  *angle that day.
  ** For big longitudes, it's quite likely the sun is up at midnight UTC: this
  *means we have to calculate successive days.
  */
  ...
  // Calculate duration (seconds) from "now" to "midnight UTC on the target
  // day". [difftime (end, beginning)]
  long waitMidnightUTC =
      static_cast<long>(difftime(pRun->targetTimet, pRun->nowTimet));

  // Calculate duration to wait for each day's rise and set (seconds)
  // (targetTimet is set to midnight on the target day)
  long waitRiseYesterday =
      waitMidnightUTC +
      static_cast<long>(3600.0 * getOffsetRiseHourUTC(pRun, &yesterday));
  long waitNoonYesterday =
      waitMidnightUTC + static_cast<long>(3600.0 * yesterday.southHourUTC);
  long waitSetYesterday =
      waitMidnightUTC +
      static_cast<long>(3600.0 * getOffsetSetHourUTC(pRun, &yesterday));
  long waitRiseToday =
      waitMidnightUTC +
      static_cast<long>(3600.0 * getOffsetRiseHourUTC(pRun, &today));
  long waitNoonToday =
      waitMidnightUTC + static_cast<long>(3600.0 * today.southHourUTC);
  long waitSetToday =
      waitMidnightUTC +
      static_cast<long>(3600.0 * getOffsetSetHourUTC(pRun, &today));
  long waitRiseTomorrow =
      waitMidnightUTC +
      static_cast<long>(3600.0 * getOffsetRiseHourUTC(pRun, &tomorrow));
  long waitNoonTomorrow =
      waitMidnightUTC + static_cast<long>(3600.0 * tomorrow.southHourUTC);
  long waitSetTomorrow =
      waitMidnightUTC +
      static_cast<long>(3600.0 * getOffsetSetHourUTC(pRun, &tomorrow));
  ...
  if (waitNoonYesterday < 0) {
    waitNoonSeconds = waitNoonToday;
  }
  if (waitNoonToday < 0) {
    waitNoonSeconds = waitNoonTomorrow;
  }
  ...
  // Determine if the day is "normal" (where the rises and sets) or "polar"
  // ("midnight sun" or "polar night")

  bool exitPolar = false;

  if (waitSetYesterday > 0) {
    double diurnalArc = diurnalArcWithOffset(pRun, &yesterday);
    exitPolar         = diurnalArc <= 0.0 || diurnalArc >= 24.0;
  } else if (waitSetToday > 0) {
    double diurnalArc = diurnalArcWithOffset(pRun, &today);
    exitPolar         = diurnalArc <= 0.0 || diurnalArc >= 24.0;
  } else {
    double diurnalArc = diurnalArcWithOffset(pRun, &tomorrow);
    exitPolar         = diurnalArc <= 0.0 || diurnalArc >= 24.0;
  }

  if (exitPolar) {
    if (pRun->debug == ONOFF_ON)
      printf("Debug: Polar region or large offset: No sunrise today, there's "
             "nothing to wait for!\n");
    return EXIT_ERROR;
  }

  // Get next rise or set time UNLESS the opposite event happens first (unless
  // less than 6 hours to required event) IF both rise and set requested THEN
  // wait for whichever is next

  long waitSeconds = 0;

  if (pRun->reportSunrise == ONOFF_ON && pRun->reportSunset == ONOFF_OFF) {
    if (isDay == ONOFF_OFF || waitRiseSeconds < 6 * 60 * 60)
      waitSeconds = waitRiseSeconds;
  } else if (pRun->reportSunrise == ONOFF_OFF &&
             pRun->reportSunset == ONOFF_ON) {
    if (isDay == ONOFF_ON || waitSetSeconds < 6 * 60 * 60)
      waitSeconds = waitSetSeconds;
  } else if (pRun->reportSolarnoon == ONOFF_ON) {
    waitSeconds = waitNoonSeconds;
  } else {
    waitSeconds =
        waitRiseSeconds < waitSetSeconds ? waitRiseSeconds : waitSetSeconds;
  }

  // Don't wait if event has passed (or next going to occur soon [6hrs])
  if (waitSeconds <= 0) {
    if (pRun->debug == ONOFF_ON)
      printf("Debug: Event already passed today, can't wait for that!\n");
    return EXIT_ERROR;
  }

  //
  // In debug mode, we don't want to wait for sunrise or sunset. Wait a minute
  // instead.
  //
  ...
  if (pRun->debug == ONOFF_ON) {
    ...
    waitSeconds = 10;
  } else if (pRun->functionPoll == ONOFF_ON)
    waitSeconds +=
        60; // Make more sure that a subsequent POLL works properly (wink ;-)
  ...
  Sleep(
      (DWORD)waitSeconds); // Windows-only . waitSec is tested positive or zero
#endif
/* Windows code: End */
