...
typedef enum { ONOFF_ON, ONOFF_OFF } OnOff;

typedef struct {
  double latitude;  // Degrees N - Global position
  double longitude; // Degrees E - Global position

  double offsetHour;    // Unit: Hours. Adjust sunrise or sunset by this amount,
                        // towards midday.
  double twilightAngle; // Degrees. -ve = below horizon. Twilight angle
                        // requested by user.
  ...
  unsigned int listDays; // How many days should sunrise/set be listed for.
                         // (function: List)
...
typedef struct {
  // "Input" data
  double twilightAngle; // Degrees. -ve = below horizon. Can be: daylight,
                        // civil, nautical, astronomical, or custom.
  unsigned long daysSince2000; // The sunrise calculation needs this: days from
                               // start of 2000 to targetTimet

  // "Output" data
  double diurnalArc;   // Target day: The time it takes the sun to travel across
                       // the sky. "southHourUTC" is mid-way.
  double southHourUTC; // Target day: Sun directly south - time (hours) from
                       // targetTimet (midnight, target Day UTC)
} targetStruct;
...
#define DAYS_TO_2000                                                           \
  365 * 30 + 7 // Number of days from 'C' time epoch (1/1/1970 to 1/1/2000)
               // [including leap days]
